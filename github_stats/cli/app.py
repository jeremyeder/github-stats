...
from ..utils import get_db, init_db, setup_logging
...
def track_org(
    ...
    with GitHubClient() as client:
        ...
        if org_info["exists"]:
            ...
            if org_info["error"]:
                console.print(f"[yellow]⚠[/yellow] Organization '{org_name}' may not exist as an organization, but tracked locally")
                console.print(f"[dim]Error: {org_info['error']}[/dim]")
            else:
                console.print(f"[green]✓[/green] Organization tracked locally: {org_info['name']}")
            ...
            try:
                ...
                for repo in repos:
                    ...
                    if repo_info["exists"]:
                        ...
                        total_interactions = len(commits) + len(issues) + len(prs) + len(stars) + len(forks) + len(releases) + len(workflows)
                        console.print(f"[green]✓[/green] {repo['full_name']} - {total_interactions} total interactions")
                    else:
                        console.print(f"[yellow]⚠[/yellow] {repo['full_name']} - basic tracking only")
                
                console.print(f"[bold green]Tracked {len(repos)} repositories with full data[/bold green]")
            except Exception as e:
                console.print(f"[red]Error fetching repositories: {e}[/red]")
        elif fetch_repos:
            console.print("[yellow]Cannot fetch repositories for non-existent organization[/yellow]")
...
def track_repo(
    ...
    if "/" in repo:
        ...
        if not org:
            console.print("[red]Error: Must provide --org or use owner/repo format[/red]")
            raise typer.Exit(1)
        owner = org
        repo_name = repo
    ...
    with GitHubClient() as client:
        ...
        if repo_info["exists"]:
            console.print(f"[green]✓[/green] Repository tracked: {repo_info['full_name']}")
            ...
            console.print(f"[green]✓[/green] Tracked {len(commit_interactions)} commits")
            ...
            console.print(f"[green]✓[/green] Tracked {len(pr_interactions)} pull requests")
            ...
            console.print(f"[green]✓[/green] Tracked {len(release_interactions)} releases")
            
            console.print("[bold]Fetching workflow runs...[/bold]")
            workflow_interactions = tracker.track_workflow_runs(owner, repo_name)
            console.print(f"[green]✓[/green] Tracked {len(workflow_interactions)} workflow runs")
        else:
            if repo_info["error"]:
                console.print(f"[yellow]⚠[/yellow] Repository '{repo}' may not exist, but tracked locally")
                console.print(f"[dim]Error: {repo_info['error']}[/dim]")
            else:
                console.print(f"[green]✓[/green] Repository tracked locally: {repo_info['full_name']}")
            console.print("[yellow]Cannot fetch data for non-existent repository[/yellow]")


@app.command()
def stats(
    org: Optional[str] = typer.Option(None, "--org", "-o", help="Filter by organization"),
    ...
    with get_db() as db:
        ...
        if interaction_type:
            try:
                ...
                console.print(f"[red]Invalid interaction type: {interaction_type}[/red]")
                raise typer.Exit(1)
...
def send_report(
    ...
    org: Optional[str] = typer.Option(None, "--org", "-o", help="Filter by organization"),
    repo: Optional[str] = typer.Option(None, "--repo", "-r", help="Filter by repository"),
    subject: Optional[str] = typer.Option(None, "--subject", "-s", help="Custom email subject"),
):
