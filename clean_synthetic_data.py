...
def clean_synthetic_data():
    ...
    with get_db() as session:
        ...
        for itype in sorted(all_types):
            ...
            print(
                f"  {itype}: {real_count} real, {synthetic_count} synthetic"
                f" (total: {total})"
            )
        ...
        if real_timestamp_interactions:
            session.commit()
            print(
                f"✅ Applied real timestamps to "
                f"{len(real_timestamp_interactions)} interactions"
            )
...
def create_star_summary():
    ...
    with get_db() as session:
        ...
        star_counts = (
            session.query(
                Repository.full_name, func.count(Interaction.id).label("star_count")
            )
            .join(Interaction, Repository.id == Interaction.repository_id)
            .filter(Interaction.type == InteractionType.STAR)
...
def show_final_data_summary():
    ...
    with get_db() as session:
        ...
        type_counts = (
            session.query(Interaction.type, func.count(Interaction.id).label("count"))
        ...
        if total_interactions > 0:
            ...
            daily_counts = (
                ...
                .order_by(func.date(Interaction.timestamp).desc())
                .limit(10)
                .all()
            )
...
def main():
    ...
    if synthetic_count > 0:
        ...
        print(
            "   Visualizations will show actual activity patterns, not synthetic data."
        )
    else:
        print("✅ No synthetic data found - your data is already clean!")
